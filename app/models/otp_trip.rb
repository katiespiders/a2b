class OTPTrip < Trip
	...
	def directions(legs)
		...
		legs.each_with_index do |leg, i|
			...
			Rails.logger.debug "#{leg['mode']} leg #{i} starts at #{Time.at(start_time)}"
	...
	def summary(legs)
		...
		legs.each do |leg|
			Rails.logger.debug leg.mode, leg.route
			if leg.mode == 'WALK'
				first_walk ||= leg
				last_walk = leg
		...
		Rails.logger.debug "first bus #{first_transit.start_time} to #{first_transit.end_time}"
		Rails.logger.debug "last bus #{last_transit.start_time} to #{last_transit.end_time}"
		...
		Rails.logger.debug "walk ends #{trip_ends}, trip middle #{trip_middle}, walk middle #{walk_middle}"
	...
	def routes(origin, destination)
		...
		Rails.logger.debug "0 s: finding transit routes"
		rts = HTTParty.get(url)['plan']
		Rails.logger.debug "#{Time.now - time} s: done with transit routes"
		rts
	end
end
